...
class _PublicProductDetailsPageState extends State<PublicProductDetailsPage>
  ...
  bool _isExpanded = false;
  int _selectedQuantity = 1;
  final Set<int> _addingToCart = {}; // Track products being added to cart
  ...
  Future<void> _addToCart() async {
    if (_product == null) return;
    if (_addingToCart.contains(_product!.id)) return;

    setState(() {
      _isAddingToCart = true;
      _addingToCart.add(_product!.id);
    });
    _buttonAnimationController.forward();
    ...
    try {
      ...
      final updatedCart = await _cartService.addItem(
        CartItemRequest(productId: _product!.id, quantity: _selectedQuantity),
      );
      ...
      if (!mounted) return;
      _showSnackBar('Failed to add ${_product!.name} to cart', isError: true);
    } finally {
      if (mounted) {
        setState(() {
          _isAddingToCart = false;
          _addingToCart.remove(_product!.id);
        });
        _buttonAnimationController.reverse();
      }
    }
  ...
  void _buyNow() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => CheckoutPage(cartItems: [_product!])),
    ).then((_) {
      if (mounted) {
        setState(() => _isBuyingNow = false);
      }
  ...
  void _incrementQuantity() {
    if (_selectedQuantity < (_product?.stockQuantity ?? 10)) {
      setState(() {
        _selectedQuantity++;
      });
    }
  ...
  Widget _buildBottomBar() {
    final isInStock = (_product!.stockQuantity ?? 0) > 0;
    final isAdding = _addingToCart.contains(_product!.id);
      ...
      child: SafeArea(
        child: Row(
          children: [
            ...
            //       ),
            //     ),
            //     // TODO: Add cart count badge if needed
            //   ],
            // ),
            const SizedBox(width: 12),
            ...
            Expanded(
              ...
              child: ScaleTransition(
                ...
                child: ElevatedButton.icon(
                  onPressed: isInStock && !isAdding ? _addToCart : null,
                  icon: isAdding
                      ? const SizedBox(
                          width: 20,
                          height: 20,
            ...
            Expanded(
              ...
              child: ScaleTransition(
                ...
                child: ElevatedButton(
                  onPressed: isInStock && !_isBuyingNow ? _buyNow : null,
                  style: ElevatedButton.styleFrom(
